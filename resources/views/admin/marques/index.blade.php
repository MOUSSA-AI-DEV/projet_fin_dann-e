                    ...
                    @foreach($marques as $marque)
                        <tr style="border-bottom: 1px solid #f1f5f9; transition: background 0.2s;">
                            <td style="padding: 1rem;">
                                ...
                                @else
                                    <div style="width: 32px; height: 32px; background: #f1f5f9; display: flex; align-items: center; justify-content: center; border-radius: 4px; color: #94a3b8; font-size: 0.7rem;">Empty</div>
                                @endif
                            </td>
                            <td style="padding: 1rem; font-weight: 500;">{{ $marque->nom }}</td>
                            <td style="padding: 1rem;">
                                @if($marque->is_active)
                            ...
                            <td style="padding: 1rem; text-align: right; display: flex; gap: 0.5rem; justify-content: flex-end;">
                                <a href="{{ route('admin.marques.edit', $marque) }}" style="padding: 5px 10px; background: #f1f5f9; border-radius: 4px; color: #475569; text-decoration: none; font-size: 0.8rem;">Modifier</a>
                                
                                <form action="{{ route('admin.marques.destroy', $marque) }}" method="POST" style="display:inline;" onsubmit="return confirm('Êtes-vous sûr ?');">
                                    @csrf
                                    @method('DELETE')
